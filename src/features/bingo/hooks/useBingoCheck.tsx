 ...
 */
export function useBingoCheck(tiles: boolean[], size = 5) {
  return useMemo(() => {
    if (!tiles?.length) return { hasBingo: false, bingoLines: [] };
    ...
    const bingoLines = lines.filter(allMarked);
    return { hasBingo: bingoLines.length > 0, bingoLines };
  }, [tiles, size]);
}
